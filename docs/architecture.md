...
flowchart LR
    ...
    genai[Google Gen AI<br/>LLM + embeddings]
    ...
    backend -->|generate grounded answer| genai
    backend -->|stream answer + citations| browser

    corpus --> ingestion
    ingestion -->|create embeddings| genai
...
- Google Gen AI SDK for generation and embeddings
...
The frontend is responsible for user interaction, local UI state, and sending the authenticated user's request to the backend. It should never hold service-role credentials, run retrieval logic, call Google Gen AI directly, or write privileged records to Supabase.
...
1. Embed the user's query with the configured Google Gen AI embedding model.
...
- No direct GENAI calls from the browser.
